...
window.onload = ()=>{
	...
	buffAudio(ngeurl, "music", "arraybuffer", cb=()=>{
		...
		start.classList.remove("linkOff");
		let stop = document.getElementById('stop');
		stop.classList.add("SS13");
...
function playbwoink(note, time = 0, sayid) {
	if(time < ctx.currentTime && time != 0){
		console.log("no.")
		return
	}
...
function record(timeat = 0) {
	...
	tostop.add(ngesrc);
	ngesrc.onended = () => {tostop.delete(ngesrc); isplaying = false;};
...
document.addEventListener("keydown", (e) => {
	...
	if(isrecording){
		recording.push({"note":note, "delay":ctx.currentTime-recordtime});
	}
});
...
setInterval(() => {
	if(isplaying)
		document.getElementById('timer').innerHTML = (isplaying ? Math.round(ctx.currentTime-recordtime) : 0) + "s";
}, 500);