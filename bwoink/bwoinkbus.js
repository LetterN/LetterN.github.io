const bwoinkrate = 0.6170518856924528;
...
var keytonote = {};
let isrecording = false, voinkkeybord = true;
//audio vars
let mainContext = new AudioContext();
var mainGNodeCtx = mainContext.createGain();
var bwoinkbuf, catbuf;

window.onload = ()=>{
	buffAudio(document.getElementById('ogg').value, "bwoink");
	buffAudio(ngeurl, "music", "arraybuffer", cb=()=>{
		let a = document.getElementById('start');
		a.classList.add("SS13");
		a.classList.remove("linkOff");
...
function buffAudio(url, who, restype="arraybuffer", cb=()=>{}){ //not blocking.
	...
	request.onload = () => {
		mainContext.decodeAudioData(request.response, 
		(buf)=>{
			if(who == "bwoink"){ //html REEEEEE
				bwoinkbuf = buf;
				console.log(bwoinkbuf);
			}
			else if(who == "music"){
				catbuf = buf;
				console.log(catbuf);
...
/**
 * Change an audiobuffer voulume
 * @param {Number} byhowmuch - from 0 to 1
 * @param {AudioBuffer} buffer 
 */
function changeVolume(byhowmuch, buffer){
	var klamp = Math.max(Math.min(byhowmuch, 1), 0);
	buffer.connect(mainGNodeCtx);
	mainGNodeCtx.connect(mainContext.destination);
	mainGNodeCtx.gain.setValueAtTime(byhowmuch, buffer.currentTime);
}

var tostop = new Set();
/**
 * Plays a bwoink
 * @param {String} note - string?
 * @param {Number} time - time on when to start
 * @param {String} sayid 
 */
function playbwoink(note, time = 0, sayid = undefined) {
	if(time < mainContext.currentTime && time != 0){
		console.log("no.");
		return
	}
	let src = mainContext.createBufferSource();
	src.buffer = bwoinkbuf;
	src.playbackRate.value = bwoinkrate;
	src.detune.value = note*100;
	src.connect(mainContext.destination);
	src.start(time);
	tostop.add(src);
	src.onended = ()=>{
		tostop.delete(src);
	...
	if(sayid != undefined){
		...
		}, 1000 * (time - mainContext.currentTime));
	}
}
...
function record(timeat = 0){
	if(isNaN(timeat)){//not a number? bye!
		timeat = 0;
	}
	let start = document.getElementById('start');
	start.classList.add("linkOn");
	start.classList.remove("SS13");
	if(isplaying){ //:rem: Removes fun
		return
	}
	isplaying = true;
	recordtime = mainContext.currentTime - timeat;
	var ngesrc = mainContext.createBufferSource();
	ngesrc.buffer = catbuf;
	ngesrc.connect(mainContext.destination);
	ngesrc.start(0, timeat);
	tostop.add(ngesrc);
	ngesrc.onended = ()=>{
		tostop.delete(ngesrc);
		isplaying = false;
	};
...
function stop(){
	let start = document.getElementById('start');
	start.classList.remove("linkOn");
	start.classList.add("SS13");
...
document.addEventListener("keydown", (e) => {
	...
	playbwoink(note, 0, `Key:${e.key}, Note:${keytonote[e.key]}`);
	if(isrecording){
		recording.push({"note":note, "delay":mainContext.currentTime-recordtime});
	}
});

function addbwoink() {
	var bwoinkdiv = document.createElement("div"); //maek div
	bwoinkdiv.classList.add("bwoinkdiv"); //add to div
	bwoinkdiv.innerHTML = bwoinklist[Math.floor(Math.random()*bwoinklist.length)]; //get random from 0 to x
	
	bwoinkdiv.style[Math.random() < .5 ? "top" : "bottom"] = `${Math.random()*360}px`; //why not px'd though??
	bwoinkdiv.style[Math.random() < .5 ? "left" : "right"] = `${Math.random()*540}px`;
	...
	setTimeout(()=>{
		bwoinkdiv.style.transform = `translate(${Math.random()*100-0}px, ${Math.random()*100-0}px) rotate(${Math.random()*20-10}deg)`;
	}, 16.6667);
	setTimeout(()=>{
		bwoinkwindow.removeChild(bwoinkdiv)
	}, 5000);
}
...
setInterval(() => {
	if(isplaying)
		document.getElementById('timer').innerHTML = (isplaying ? Math.round(mainContext.currentTime-recordtime) : 0) + "s";
}, 500);